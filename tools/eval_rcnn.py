...
def eval_one_epoch_joint(model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        ...
        ######## 2022.9.2
        # rpn_img_score_map = ret_dict['rpn_image_seg'].permute(0,2,3,1).squeeze(0).detach().cpu().numpy()
        # rpn_img_feat_map = ret_dict['img_feature'].permute(0,2,3,1).squeeze(0).detach().cpu().numpy()
        # str_cls_name = cfg.CLASSES
        # #print('####sample_id:', cnt-1)
        # print('####sample_id:', sample_id)
        # result_dir = '/data2/zheliu/TPAMI_rebuttal_2022'
        # final_output_pretrained_img_scores_dir = os.path.join(result_dir, 'img_output', 'pretrained_img_scores', str_cls_name)
        # os.makedirs(final_output_pretrained_img_scores_dir, exist_ok = True)
        # final_output_pretrained_img_feats_dir = os.path.join(result_dir, 'img_output', 'pretrained_img_feats', str_cls_name)
        # os.makedirs(final_output_pretrained_img_feats_dir, exist_ok = True)
        # np.save(final_output_pretrained_img_scores_dir+'/'+'%04d'%sample_id+'.npy', rpn_img_score_map) ## have saved!!!
        #np.save(final_output_pretrained_img_feats_dir+'/'+str(sample_id)+'.npy', rpn_img_feat_map)

        ##################################################
        if cfg.USE_IMAGE_SCORE:
            final_output_point_score_dir = os.path.join(result_dir, 'final_result', 'data_point_scores')
            os.makedirs(final_output_point_score_dir, exist_ok = True)
            final_output_image_score_dir = os.path.join(result_dir, 'final_result', 'data_image_scores')
            os.makedirs(final_output_image_score_dir, exist_ok = True)

            rpn_point_scores = ret_dict['rpn_point_scores']
            rpn_image_scores = ret_dict['rpn_image_scores']
            a11 = rpn_point_scores.data.cpu().numpy()
            b11 = rpn_image_scores.data.cpu().numpy()
            # np.save(final_output_point_score_dir+'/point_scores'+str(cnt-1)+'.npy', a11)
            # np.save(final_output_image_score_dir+'/img_scores'+str(cnt-1)+'.npy', b11)
        ##################################################
...
def eval_single_ckpt(root_result_dir,data_path):
    ...
    test_loader = create_dataloader(logger, data_path)
...
def repeat_eval_ckpt(root_result_dir, ckpt_dir, data_path):
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        if args.eval_all:
            ...
            repeat_eval_ckpt(root_result_dir, ckpt_dir, data_path=args.data_path)
        else:
