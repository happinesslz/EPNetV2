...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        if cfg.DEEP_RCNN_FUSION:
            pts_extra_input_list.append(input_dict['l_xy_cor'])

        if cfg.RCNN.USE_INTENSITY:
            pts_extra_input_list.append([input_dict['rpn_intensity'].unsqueeze(dim = 2),
                                    input_dict['seg_mask'].unsqueeze(dim = 2)])
        else:
            pts_extra_input_list.append(input_dict['seg_mask'].unsqueeze(dim = 2))
        ...
        if cfg.DEEP_RCNN_FUSION:
            sampled_pts, sampled_xy_cor, sampled_features = pooled_features[:, :, :, 0:3], \
                                                            pooled_features[:, :, :,3:5], pooled_features[:, :,:, 5:]
        else:
